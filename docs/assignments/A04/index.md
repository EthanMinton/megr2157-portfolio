...
Another lesson that I learned is that using the global parametric variables works well for scaling basic bracket dimensions (such as the base width "b" or overall height "h"), but while researching, small gear motors rarely scale properly across different models. With Varied structure lips that contain different intricacies in bolt placement, shaft structure, and the overall connection. It's difficult to create a model that works with a singular motor, let alone be adaptive with many. Although I would still always opt for parametric design, as it allows us to easily adapt dimensions that we could want to change, such as the possibility of changing the base to 25 mm and recognizing whether or not this causes the model to fail or change. Oftentimes it is difficult to observe how an idea will scale out, but with parametric design, it can allow us to really see the images of merely changing a single variable on a whole design.
